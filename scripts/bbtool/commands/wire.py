...
def _wildcard_last_providers(entries: List[InitEntry]) -> List[InitEntry]:
...
def render_source(ordered: List[InitEntry], provides_entries: List[ProvidesEntry] = ()) -> str:
    ...
    Raises WireError if:
      ...
      - an entry's `out=` varname collides with codegen's own reserved
        ...
        collide with a generated file-scope declaration (compile error); or
    ...
    # re-derived raw (order, parse-index) tie-key) -- `ordered`'s own position
    # already encodes both the tier barrier (early entries all precede every
    # pre_http entry, which all precede every regular entry, regardless of
    # order=) and the same-tier requires=/provides=/tie-break resolution
    # wire_graph.topo_sort performs, so checking list position here is
    # strictly more accurate than recomputing the tie-key in isolation and
    # needs no extra tier-boundary special-casing.
    wildcard_providers = _wildcard_last_providers(ordered)
    if len(wildcard_providers) > 1:
        offenders = ", ".join(f"{e.fn} ({e.src_file}:{e.src_line})" for e in wildcard_providers)
        raise WireError(
            f"more than one entry provides '{WILDCARD_LAST_PROVIDES_KEY}' "
            f"({offenders}) — exactly one is required"
        )

    wildcard_entry = wildcard_providers[0] if wildcard_providers else None
        ...
        for i, e in enumerate(ordered):
            ...
            if i >= wildcard_idx:
                ...
                raise WireError(
                    ...
                    f"'{WILDCARD_LAST_PROVIDES_KEY}' -- consumer routes must "
                    f"register before the wildcard-catching entry, or they "
                    f"are never reachable; give {e.fn} an explicit order= "
                    f"(or reorder its marker) so it sorts before {wildcard_entry.fn}"
                )
