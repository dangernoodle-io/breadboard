...
class TestEmitSeamUnwiredSubscriber(unittest.TestCase):
    ...
    def test_real_repo_clean_smoke_and_floor(self):
        ...
        includes bb_wifi + bb_mdns/bb_mqtt_client but IS wired via codegen
        (bb_event.h's `// bbtool:provides key=emit_sink` + bb_wifi.h's `//
        bbtool:init ... consumes=emit_sink`, emitted into
        examples/smoke/main/generated/bb_app_init.c -- regenerated by the
        Makefile's smoke-gen/floor-gen, a prerequisite of this test's own
        `test-py` target) -- so the real repo must be clean under this
        rule."""
