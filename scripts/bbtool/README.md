...
| `public-header-leak` | library | Flags `esp_*/driver//cJSON.h/pthread.h` includes in public component headers outside an `#ifdef ESP_PLATFORM` gate |
...
Freezes breadboard's pre-existing `-Wall` warning backlog shrink-only
(B1-1420): the repo sets `-Werror` unconditionally but never `-Wall`/
`-Wformat`, so a genuine GNU toolchain that ships a different default
warning set (Ubuntu's gcc patches `-Wformat` on, Homebrew's does not) can
local-clean/CI-fail (PR #1218). `-Wall` cannot simply be flipped on yet — a
real `-Wall` build still reproduces a backlog that hasn't been cleared
file-by-file — so this ratchets that backlog instead, the same way
`di_legacy` ratchets legacy DI glue.

**These two families are structurally different from every other family
above.** Every other family's `_scan_*` walks `components/`/`platform/`
source text with a regex — cheap, deterministic, no toolchain needed. A
`-Wall` diagnostic can only come from an actual compile, and the two
targets need two different toolchains (genuine-GNU host gcc,
xtensa/ESP-IDF) neither of which the `check` CI job has (bare `make check`
on ubuntu-latest, no PlatformIO at all). So each family's scanner instead
reads a **live, gitignored build-artifact report**
(`.baseline/bbtool/fence/warning_fence_reports/{host,firmware}.log`,
regenerated by `scripts/warn_scan.sh <host|firmware>`) rather than scanning
source directly. Split into two families (not one combined) because host
and firmware are captured by two different CI jobs — a single family would
wrongly hard-fail in whichever job's report is absent from that job's own
checkout. Shared parsing + report contract: `fence/_gcc_warnings.py`.

**Report contract.** `warn_scan.sh` builds with `-Wall -Wno-error`
temporarily overriding the committed `-Werror` (never editing
`platformio.ini`/`examples/smoke/CMakeLists.txt` on disk — override at
invocation only, restored via a trap even on failure) and writes a report
whose first line is always `# bb-warn-scan v1 target=<T>
build_exit=<N>`, followed by the build's `warning:`/`In function`/`At top
level` diagnostic lines. Marker identity is `(warning flag, file,
enclosing-function-or-file-scope + message)` — deliberately excludes line
number (an edit that shifts lines above a warning must not spuriously trip
the fence) and is symbol/scope-anchored like every other family's identity.

**Enforcement is gated on `BB_WARNING_FENCE_STRICT=1`, not plain scan
success/failure.** `.github/workflows/build.yml`'s `test` job (host — the
job that already builds natively for coverage) and the `smoke` job's
`esp32` matrix leg (firmware) each run `warn_scan.sh <target>` then
`BB_WARNING_FENCE_STRICT=1 fence --family warning_fence_<target>`
immediately after, in the same job — a new warning fails the build where
it's produced. A report that is missing, empty, unreadable, has a
malformed header, or carries a non-zero `build_exit` (the capture build
itself broke) is a **hard fail** there (`WarningReportInvalidError`) —
absence of evidence is never "zero warnings". Everywhere else — the
toolchain-less `check` job, `make test-py`, a dev machine without
PlatformIO/genuine-GNU gcc, or simply not having run `warn_scan.sh` —
`BB_WARNING_FENCE_STRICT` is unset, and the same condition is a loud `SKIP`
to stderr instead, scanning as zero markers: `make check`/`make fence` stay
usable without a toolchain, but never look like they validated something
they didn't.

**Firmware enforcement gap, stated precisely.** The `smoke` job (and so
`warning_fence_firmware`) only runs on `pull_request` events whose diff
touches a firmware-classified path (the `changes` job's path filters) — it
never runs on a direct `push` to `main`, nor on a PR that doesn't touch
those paths. Within a `smoke` run, `warning_fence_firmware`'s job leg is
`optional` in the `summary` gate's `ci-result-gate` action, but optional
there means "success-or-skipped blocks merge on any real failure just like
a required check" — a genuine regression still blocks merge whenever the
job runs; the actual gap is purely the "does it run at all" question above,
not a weaker pass bar when it does. `warning_fence_host` (the `test` job)
has no such gap — it's `required` and runs on every push and PR.

**`--update-baseline` and SKIP interact deliberately, unlike every other
family.** The generic `diff()`/prune path assumes "current scan found
fewer markers than baseline" means genuine shrinkage — true for every
filesystem-walking family, but wrong here: a SKIP-mode scan legitimately
returns an empty set for "not scanned here", which `diff()` alone cannot
tell apart from "genuinely zero warnings". Both families define a
`scan_is_trustworthy(root) -> bool` hook (backed by
`_gcc_warnings.is_report_valid`) that `commands/fence_cmd.py`'s
`_update_baseline` consults before pruning — a SKIP-mode
`--update-baseline` run is a no-op (`baseline left untouched`), never a
prune, even though a plain (non-`--update-baseline`) `fence` check under
SKIP still safely PASSes (an empty `current` can never produce a `new`
marker either) — its would-be "candidate to prune" noise is summarized
into one INFO line instead of one per baseline entry, since none of them
are real prune candidates. Every other family has no `scan_is_trustworthy`
hook and is unaffected (default: always trustworthy, unchanged behavior).

```
scripts/warn_scan.sh host        # regenerate the host report locally
scripts/warn_scan.sh firmware    # regenerate the firmware report locally (patches
                                  # examples/smoke/CMakeLists.txt only for the build's
                                  # duration, restores it via a trap)
python3 scripts/bbtool.py fence --family warning_fence_host
```

See `scripts/bbtool/fence/_gcc_warnings.py` (shared parsing + report
contract + `is_report_valid`), `scripts/bbtool/fence/warning_fence_host.py`
/ `warning_fence_firmware.py` (the two thin family modules), and
`scripts/warn_scan.sh` (report generator, also the exact command CI runs).
