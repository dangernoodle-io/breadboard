...
CLI usage (for cmake/bb_version.cmake and other non-PlatformIO callers):
  python bb_version.py --emit --consumer <consumer_dir> [--bb-dir <bb_dir>]
  Prints ONLY the bare version string to stdout (no log lines).
"""
...
# Capture the script's real path at module load time (before any function calls).
# `__file__` is NOT set when SCons exec()'s the script, so we use inspect to
# recover the absolute path from the current frame's code-object filename.
...
def _cli_main():
    """Standalone CLI: print the bare version string to stdout.

    Usage: python bb_version.py --emit --consumer <dir> [--bb-dir <dir>]
    """
    import argparse
    ...
    parser.add_argument("--bb-dir",
                        help="breadboard repo root (default: parent of this script)")
    args = parser.parse_args()

    bb_dir = os.path.abspath(args.bb_dir) if args.bb_dir else os.path.abspath(os.path.join(_THIS_DIR, ".."))
...
# ---------------------------------------------------------------------------
# PlatformIO / SCons pre-script entrypoint
# ---------------------------------------------------------------------------
...
def _pio_main(env):
    ...
    # breadboard repo = parent of this script's directory.
    # _THIS_DIR is captured at module load time (inspect-based, handles SCons exec).
...
# ---------------------------------------------------------------------------
# dispatch: CLI vs SCons
# ---------------------------------------------------------------------------

if __name__ == "__main__":
    _cli_main()
else:
    # Running under SCons/PlatformIO: env is injected via Import("env").
    try:
        Import("env")  # noqa: F821 — SCons builtin, not available in CLI
        _pio_main(env)  # noqa: F821 — populated by Import above
    except NameError:
        # Not running under SCons — nothing to do (e.g. imported as a module).
        pass
