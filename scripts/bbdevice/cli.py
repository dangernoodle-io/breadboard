...
from bbdevice.core import load_config, load_plugins
from bbdevice.registry import COMMANDS, PluginAPI
from bbdevice.commands import FIRST_PARTY


def main() -> int:
    # Pre-parse --root/--config before building subparsers so we can locate
    # bbdevice.toml (and thus external plugin paths) before argparse sees the
    ...
    # Load first-party commands onto the bus first (empty today — device
    # layer lands in a later PR).
    ...
    parser = argparse.ArgumentParser(
        prog="bbdevice",
        description="breadboard device HTTP/WS test-and-ops CLI framework",
    )
    parser.add_argument(
        "--root",
        default=os.getcwd(),
    ...
    parser.add_argument(
        "--config",
        default=None,
        help="path to bbdevice.toml (default: <root>/bbdevice.toml)",
    )
