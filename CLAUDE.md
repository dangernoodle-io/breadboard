...
**Store-and-forward ring buffer (`BB_PUB_BUFFER_ENABLE`).** The ring is backed by a `bb_pool` FIFO carved from a `bb_pub`-private static-BSS `bb_arena` (B1-419 / B1-478 PR D) — there is no heap allocation for this ring, ever, on either mode. The arena is sized once via `BB_PUB_RING_POOL_ARENA_BYTES` (512 B pool/arena struct headroom + 64 B per-entry overhead + `MAX_ENTRIES × ENTRY_MAX`) and is a **permanent BSS reservation** — roughly **~8.5 KB at Kconfig defaults** (`512 + 16×64 + 16×449` — `ENTRY_MAX` = `TOPIC_MAX`(192) + 1 + `MAX_PAYLOAD_BYTES`(256) = 449) — whenever `CONFIG_BB_PUB_BUFFER_ENABLE=y`, **regardless of `BB_PUB_BUFFER_ALWAYS`**. The two modes selected by `BB_PUB_BUFFER_ALWAYS` differ only in *when* the pool is populated, not in whether the backing memory is reserved:
- **On-failure mode (default, `BB_PUB_BUFFER_ALWAYS=n`):** the pool is lazily populated (the arena struct/pool object is created on first sink failure, freed again after `CONFIG_BB_PUB_BUFFER_IDLE_FREE_TICKS` idle ticks post-drain) — but the underlying static-BSS arena buffer is not standing cost from the ring itself; it is still linked into BSS whenever the component is compiled in. When sink[0] fails, the serialized payload is captured. On the next tick where sink[0] succeeds, buffered entries are replayed oldest-first before live data. `captured_ms` is injected into any replayed entry that had a synced epoch at capture time (every buffered entry is by definition delayed, so injection is unconditional when `ts > 0`).
- **Always-on mode (`BB_PUB_BUFFER_ALWAYS=y`):** every sink[0] publish is enqueued into the ring and drained each tick (enqueue → drain), providing a uniform ordered queue. Sinks[1..] keep direct fan-out unchanged. The pool is created eagerly at init (`bb_pub_buffer_init_eager()`). `captured_ms` is injected ONLY when an entry is genuinely delayed across ticks: age `(now_epoch_ms − capture_epoch_ms) > 1.5 × interval_ms`. Fresh entries drained the same tick they were enqueued are below the threshold and carry no `captured_ms` field, keeping healthy-operation points clean. `last_publish_ok` in always-on mode reflects whether the ring drained fully (empty after replay), not individual direct-publish results. **Oversized-entry fallback:** if an entry's `topic_len + 1 + payload_len` exceeds the ring slot (`BB_PUB_BUFFER_ENTRY_MAX`), `buffer_capture` returns `false` and the always-on path publishes the entry directly (live) instead of dropping it — it is not buffered for replay, but the live data is preserved. This is the correct behavior for topics like `info` whose serialized payload can exceed the ring slot on no-PSRAM boards.
...
| `CONFIG_BB_PUB_BUFFER_ENABLE` | n | Enable store-and-forward ring. Compiles in `bb_pool`/`bb_arena` dependency (private static-BSS arena) and capture/replay logic. Required for both on-failure and always-on modes. Reserves ~8.5 KB BSS at defaults (see Store-and-forward ring buffer section above) regardless of `BB_PUB_BUFFER_ALWAYS`. |
...
| `CONFIG_BB_PUB_BUFFER_ALWAYS` | n | Always-on ring mode (`depends on BB_PUB_BUFFER_ENABLE`). When y: all sink[0] traffic routes through the ring (enqueue→drain each tick) for a uniform ordered queue; pool is populated eagerly at boot. When n (default): pool is populated lazily (created on first sink failure, freed after idle ticks) — same standing BSS reservation either way, only the object-creation timing differs. Recommended on PSRAM boards for always-on. `captured_ms` is injected only when an entry is delayed beyond 1.5 × interval_ms in always-on mode; unconditionally (when `ts > 0`) in on-failure mode. |
