...
**SSE pool backing (`CONFIG_BB_EVENT_ROUTES_POOL_STATIC`, B1-491).** The SSE per-client pool (queue entries + payload buffers in `bb_event_routes_common.c`, plus on ESP-IDF the writer task stack + TCB in `bb_event_routes_espidf.c`) is always index-addressed — client slot i deterministically owns pool bundle i, no free-list, no per-connect malloc/free churn (B1-478 PR E). This knob selects the pool's *backing*, not whether pooling happens:
- **n (default) — lazy heap-backed.** Both arenas (payload/entries pool and, on ESP-IDF, the task-stack pool) are allocated via a single contiguous heap allocation (SPIRAM-preferred) on the FIRST SSE client connect — a board that never gets an SSE client (headless, no dashboard) pays ≈0 standing RAM. A failed allocation (fragmented/low heap) fails soft: the connection is rejected and retried on the next connect, never a crash or unchecked NULL. The heap allocation is never freed once created — deliberate, not an oversight: the per-client bundle includes a live FreeRTOS task stack, and freeing a stack out from under a task that has not finished exiting is a use-after-free (B1-484 / PR-E-CRITICAL class). One standing heap allocation for the process lifetime is the safe trade; it still fully avoids permanent BSS on headless boards.
