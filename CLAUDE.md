...
**Verify the CI way** — host tests FALSE-PASS under macOS's default toolchain (Apple clang masquerading as `gcc`/`gcov`), so a local run needs a genuine GNU toolchain (matched `gcc`/`gcov` major version) — this is a LOCAL requirement, not a reproduction of CI (CI has never pinned a specific gcc version; `pio test` there runs under ubuntu-latest's stock gcc with no shim or version check at all). Also build a real platform target (`make smoke`), which false-passes on native too. Never trust a local green alone.
