...
1. `bb_net_health_register_health()` — before `bb_http_server_start` (before the health section table is frozen). Registers a `"net"` section on `/api/health` with fields: `rssi` (int), `state` (string: `"good"` / `"marginal"` / `"poor"`), `early_warning` (bool), `throttled` (bool), `last_disconnect_reason` (uint, WiFi), `disc_age_s` (uint), and a nested `"mqtt"` object: `{ "connected": bool, "reconnect_count": uint, "disc_age_s": uint, "disc_reason": uint, "tls_fail": uint }`.
