...
// ---------------------------------------------------------------------------
// bb_str_envelope_split — pure byte-range envelope scan for the
// {"ts_ms":N,"data":{...}} wire envelope (its former sole consumer,
// bb_cache_reactive, was deleted in B1-1118 -- kept as public API, rehomed
// from bb_json to bb_str in B1-861 so it survives bb_json's eventual
// deletion).
// ---------------------------------------------------------------------------
...
void test_bb_str_envelope_split_basic_shape(void)
{
    ...
    TEST_ASSERT_TRUE(bb_str_envelope_split(payload, (int)strlen(payload),
...
void test_bb_str_envelope_split_missing_ts_ms_returns_false(void)
{
    ...
    TEST_ASSERT_FALSE(bb_str_envelope_split(payload, (int)strlen(payload),
                                             &ts_start, &ts_len, &data_start, &data_len));
}
...
// the four whitespace characters bb_str_envelope_find_key_value's
// skip-loop recognizes.
...
void test_bb_str_envelope_split_null_args_return_false(void)
{
    ...
    TEST_ASSERT_FALSE(bb_str_envelope_split(NULL, 5, &ts_start, &ts_len, &data_start, &data_len));
    TEST_ASSERT_FALSE(bb_str_envelope_split("{}", 0, &ts_start, &ts_len, &data_start, &data_len));
    TEST_ASSERT_FALSE(bb_str_envelope_split("{}", 2, NULL, &ts_len, &data_start, &data_len));
    TEST_ASSERT_FALSE(bb_str_envelope_split("{}", 2, &ts_start, NULL, &data_start, &data_len));
