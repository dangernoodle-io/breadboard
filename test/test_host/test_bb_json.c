...
// ---------------------------------------------------------------------------
// bb_json_envelope_split — pure byte-range envelope scan (shared by
// platform/{espidf,host}/bb_sink_ws/bb_sink_ws.c; extracted here to kill the
// prior verbatim duplication across those two backends).
// ---------------------------------------------------------------------------

void test_bb_json_envelope_split_basic_shape(void)
{
    const char *payload = "{\"ts_ms\":1700000000123,\"data\":{\"v\":1}}";
    const char *ts_start = NULL, *data_start = NULL;
    size_t ts_len = 0, data_len = 0;

    TEST_ASSERT_TRUE(bb_json_envelope_split(payload, (int)strlen(payload),
                                             &ts_start, &ts_len, &data_start, &data_len));
    TEST_ASSERT_EQUAL_INT(0, strncmp(ts_start, "1700000000123", ts_len));
    TEST_ASSERT_EQUAL_UINT(strlen("1700000000123"), ts_len);
    TEST_ASSERT_EQUAL_INT(0, strncmp(data_start, "{\"v\":1}", data_len));
    TEST_ASSERT_EQUAL_UINT(strlen("{\"v\":1}"), data_len);
}

void test_bb_json_envelope_split_negative_ts_is_valid(void)
{
    const char *payload = "{\"ts_ms\":-5,\"data\":{}}";
    const char *ts_start = NULL, *data_start = NULL;
    size_t ts_len = 0, data_len = 0;

    TEST_ASSERT_TRUE(bb_json_envelope_split(payload, (int)strlen(payload),
                                             &ts_start, &ts_len, &data_start, &data_len));
    TEST_ASSERT_EQUAL_INT(0, strncmp(ts_start, "-5", ts_len));
}

void test_bb_json_envelope_split_missing_ts_ms_returns_false(void)
{
    const char *payload = "{\"data\":{\"v\":1}}";
    const char *ts_start = NULL, *data_start = NULL;
    size_t ts_len = 0, data_len = 0;

    TEST_ASSERT_FALSE(bb_json_envelope_split(payload, (int)strlen(payload),
                                              &ts_start, &ts_len, &data_start, &data_len));
}
...
{
    // "-" with no following digits: ts_end == ts_v + 1 && *ts_v == '-'.
    const char *payload = "{\"ts_ms\":-,\"data\":{}}";
    const char *ts_start = NULL, *data_start = NULL;
    size_t ts_len = 0, data_len = 0;
...
{
    const char *payload = "{\"ts_ms\":1,\"data\":[1,2]}";
    const char *ts_start = NULL, *data_start = NULL;
    size_t ts_len = 0, data_len = 0;
...
// Regression: a "data" string value containing a literal '}' must not
// desync the brace-balance scan and truncate/corrupt the returned range.
void test_bb_json_envelope_split_brace_in_string_value_is_ignored(void)
{
    const char *payload = "{\"ts_ms\":5,\"data\":{\"msg\":\"a}b\"}}";
    const char *ts_start = NULL, *data_start = NULL;
    size_t ts_len = 0, data_len = 0;

    TEST_ASSERT_TRUE(bb_json_envelope_split(payload, (int)strlen(payload),
                                             &ts_start, &ts_len, &data_start, &data_len));
    TEST_ASSERT_EQUAL_INT(0, strncmp(data_start, "{\"msg\":\"a}b\"}", data_len));
    TEST_ASSERT_EQUAL_UINT(strlen("{\"msg\":\"a}b\"}"), data_len);
}

// Regression: an escaped quote (\") immediately followed by a brace must
// still be treated as inside the string -- a naive (non-escape-aware)
// quote toggle would exit the string at the escaped quote and then
// misinterpret the following '{' as real object structure.
void test_bb_json_envelope_split_escaped_quote_before_brace_is_ignored(void)
{
    const char *payload = "{\"ts_ms\":7,\"data\":{\"msg\":\"a\\\"{b\"}}";
    const char *ts_start = NULL, *data_start = NULL;
    size_t ts_len = 0, data_len = 0;
...
// Whitespace between "key": and its value is tolerated -- exercise each of
// the four whitespace characters find_key_value's skip-loop recognizes.
...
{
    const char *payload = "{\"ts_ms\":\r5,\"data\":{}}";
    const char *ts_start = NULL, *data_start = NULL;
    size_t ts_len = 0, data_len = 0;
...
// "ts_ms": with no value at all, ending exactly at the buffer boundary --
// ts_v lands on end, so both the leading "-" check and the digit-scan loop
// see ts_end == end on their very first test.
...
// "data": with no value at all, ending exactly at the buffer boundary --
// data_v lands on end, hitting the `data_v >= end` guard directly.
void test_bb_json_envelope_split_data_value_at_buffer_end_returns_false(void)
{
    const char *payload = "{\"ts_ms\":1,\"data\":";
    const char *ts_start = NULL, *data_start = NULL;
    size_t ts_len = 0, data_len = 0;
...
// Nested object inside "data" -- the inner '}' decrements depth to a
// non-zero value, exercising the depth-still-open branch before the outer
// '}' finally brings depth back to 0.
void test_bb_json_envelope_split_nested_data_object_is_balanced(void)
{
    const char *payload = "{\"ts_ms\":1,\"data\":{\"a\":{\"b\":1}}}";
    const char *ts_start = NULL, *data_start = NULL;
    size_t ts_len = 0, data_len = 0;

    TEST_ASSERT_TRUE(bb_json_envelope_split(payload, (int)strlen(payload),
                                             &ts_start, &ts_len, &data_start, &data_len));
    TEST_ASSERT_EQUAL_INT(0, strncmp(data_start, "{\"a\":{\"b\":1}}", data_len));
    TEST_ASSERT_EQUAL_UINT(strlen("{\"a\":{\"b\":1}}"), data_len);
}
...
void test_bb_json_envelope_split_null_args_return_false(void)
{
    ...
    TEST_ASSERT_FALSE(bb_json_envelope_split(NULL, 5, &ts_start, &ts_len, &data_start, &data_len));
    TEST_ASSERT_FALSE(bb_json_envelope_split("{}", 0, &ts_start, &ts_len, &data_start, &data_len));
    TEST_ASSERT_FALSE(bb_json_envelope_split("{}", 2, NULL, &ts_len, &data_start, &data_len));
    TEST_ASSERT_FALSE(bb_json_envelope_split("{}", 2, &ts_start, NULL, &data_start, &data_len));
    TEST_ASSERT_FALSE(bb_json_envelope_split("{}", 2, &ts_start, &ts_len, NULL, &data_len));
    TEST_ASSERT_FALSE(bb_json_envelope_split("{}", 2, &ts_start, &ts_len, &data_start, NULL));
}
