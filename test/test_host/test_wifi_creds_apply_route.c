// Host exercise of the "wifi" bb_data ingress binding (B1-1022) that backs
...
// file. This file used to hand-duplicate the binding's field descriptor
// (mirror_creds_t / s_mirror_fields / s_mirror_desc) for exactly that
// reason; B1-1178 extracted the real descriptor
// (bb_wifi_http_creds_wire_t / bb_wifi_http_creds_wire_desc) into the
// portable components/bb_wifi_http/bb_wifi_http_creds_wire.c, so this file
// now binds and drives the REAL production descriptor directly -- no more
// mirror.
//
// This file drives that descriptor plus local gather/apply spies (matching
// the production hooks' shape, minus the ESP-IDF-only
// bb_wifi_reconfigure() call) through the REAL bb_data_apply() pipeline
// (JSON parse -> bb_serialize_populate() -> apply()) -- exercising the
// composed route's gather/apply pairing and field-name wiring, not just
// bb_data's own generic unit tests (test_bb_data.c). Any edit to the
// production binding's gather/apply mode must be mirrored here.
//
...
{
    (void)args;
    bb_wifi_http_creds_wire_t *creds = (bb_wifi_http_creds_wire_t *)dst;
...
static bb_wifi_http_creds_wire_t s_captured;
...
static void reset_all(void)
{
    ...
    bb_data_binding_t b = {
        .key = "wifi", .desc = &bb_wifi_http_creds_wire_desc, .gather = mirror_gather,
        .apply = mirror_apply,
    };
    ...
    bb_data_binding_t b_patch_variant = {
        .key = "wifi_patch_contrast", .desc = &bb_wifi_http_creds_wire_desc,
