...
static void parse(const char *line)
{
    ...
    s_tag[0] = '\0';
    s_msg[0] = '\0';
    bb_log_line_parse(line, strlen(line), &s_level, s_tag, sizeof(s_tag),
                      s_msg, sizeof(s_msg));
}
...
{
    bb_log_line_parse("", 0, &s_level, s_tag, sizeof(s_tag), s_msg, sizeof(s_msg));
    TEST_ASSERT_EQUAL_CHAR('?', s_level);
    TEST_ASSERT_EQUAL_STRING("", s_tag);
...
{
    s_level = 'X';
    bb_log_line_parse(NULL, 5, &s_level, s_tag, sizeof(s_tag), s_msg, sizeof(s_msg));
    TEST_ASSERT_EQUAL_CHAR('?', s_level);
}
...
void test_bb_log_line_parse_msg_truncation(void)
{
    ...
    char tag[16];
    char msg[256];
    bb_log_line_parse(line, strlen(line), &level, tag, sizeof(tag), msg, sizeof(msg));
...
void test_bb_log_line_parse_tag_truncation(void)
{
    ...
    char msg[64];
    bb_log_line_parse("I (1) longtag: hi", 17, &level, tag, sizeof(tag), msg, sizeof(msg));
    TEST_ASSERT_EQUAL_CHAR('I', level);
...
void test_bb_log_line_parse_zero_cap(void)
{
    char level;
    char tag[16];
    bb_log_line_parse("I (1) t: msg", 12, &level, tag, sizeof(tag), NULL, 0);
    TEST_ASSERT_EQUAL_CHAR('I', level);
    TEST_ASSERT_EQUAL_STRING("t", tag);
}
