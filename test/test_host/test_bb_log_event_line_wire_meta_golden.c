// test_bb_log_event_line_wire_meta_golden -- B1-1059 PR-2b-i-2: proves the
// #if-gated co-located bb_serialize_desc_meta_t pattern (B1-1059 PR-2a
// exemplar, test_bb_wifi_http_wire_meta_golden.c) for the SSE "log" topic
// (bb_log_event_line_wire_desc / bb_log_event_line_wire_meta, both in
// components/bb_log_event/src/bb_log_event_line_wire.c). Only reachable when
// BB_SERIALIZE_META_HOST is defined (this native env; see
// platformio.ini) -- the ESP-IDF/device build never defines it, so
// bb_log_event_line_wire_meta doesn't exist there.
//
// Fidelity finding vs the hand-authored k_log_event_schema literal
// (platform/espidf/bb_log_event/bb_log_event.c): the "properties" content
// (including "level"'s enum) and the "required" set are BYTE-IDENTICAL (same
// field order, same field types, all four fields required). Three structural
// differences are inherent to the meta engine's fixed schema-shape policy
// and this descriptor's SSE-topic nature, not this table's content, and are
...
//      "title":"LogEvent" is supplied via
...
// Byte-fidelity target: k_log_event_schema's "properties"/"required"
...
void test_bb_log_event_line_wire_meta_validates_against_desc(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(BB_OK,
        bb_serialize_meta_validate(&bb_log_event_line_wire_desc, &bb_log_event_line_wire_meta,
                                    err, sizeof err));
}
...
// test-local copy of either) reproduces k_log_event_schema's field-level
...
void test_bb_log_event_line_wire_meta_golden_matches_hand_literal(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(BB_OK,
        bb_serialize_meta_openapi_schema(&bb_log_event_line_wire_desc, &bb_log_event_line_wire_meta,
                                          buf, sizeof buf, &n));
