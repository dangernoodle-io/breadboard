// test_bb_health_stack_wire_meta_golden -- B1-1059 PR-2b-i-2: proves the
// #if-gated co-located bb_serialize_desc_meta_t pattern (B1-1059 PR-2a
// exemplar, test_bb_wifi_http_wire_meta_golden.c) for the SSE "health.stack"
// topic (bb_health_stack_wire_desc / bb_health_stack_wire_meta, both in
// components/bb_health/bb_health_stack_wire.c). Only reachable when
// BB_SERIALIZE_META_HOST is defined (this native env; see
// platformio.ini) -- the ESP-IDF/device build never defines it, so
// bb_health_stack_wire_meta doesn't exist there.
//
// Fidelity finding vs the hand-authored k_health_stack_schema literal
// (platform/espidf/bb_health/bb_health_stack.c): the "properties" content
// and the "required" set are BYTE-IDENTICAL (same field order, same field
// types, all three fields required). Three structural differences are
// inherent to the meta engine's fixed schema-shape policy and this
// descriptor's SSE-topic nature, not this table's content, and are accepted
// as documented deltas (not a fidelity failure):
//   1. no top-level "title" -- the composer has no descriptor-level
//      title hook (see the PR-2a exemplar's own delta #1); the hand
//      literal's "title":"HealthStack" is supplied via
...
// Byte-fidelity target: k_health_stack_schema's "properties"/"required"
...
void test_bb_health_stack_wire_meta_validates_against_desc(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(BB_OK,
        bb_serialize_meta_validate(&bb_health_stack_wire_desc, &bb_health_stack_wire_meta,
                                    err, sizeof err));
}
...
// test-local copy of either) reproduces k_health_stack_schema's field-level
...
void test_bb_health_stack_wire_meta_golden_matches_hand_literal(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(BB_OK,
        bb_serialize_meta_openapi_schema(&bb_health_stack_wire_desc, &bb_health_stack_wire_meta,
                                          buf, sizeof buf, &n));
