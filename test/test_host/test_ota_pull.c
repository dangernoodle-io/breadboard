...
// ---------------------------------------------------------------------------
// OTA claim deadlock contract — the apply handler must NOT hold the OTA claim
// while calling bb_ota_check_run_blocking(), or a concurrent upd_check worker
// deadlocks on the same claim. This contract is NOT currently exercised by
// any test (the 4 tests that claimed to were deleted, B1-1212: they only
// drove the generic bb_ota_check_ota_claim_* primitive, already covered in
// test_bb_ota_check.c, never the apply handler itself). A real regression
// test is tracked separately (B1-1216).
// ---------------------------------------------------------------------------
