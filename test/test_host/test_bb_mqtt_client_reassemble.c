// Tests for bb_mqtt_client_reasm_step (B1-487 HIGH-1): the pure fragmented-payload
...
static bb_mqtt_client_reasm_state_t  s_st;
...
static void reasm_setup(void)
{
    ...
    bb_mqtt_client_reasm_reset(&s_st);
}

void test_bb_mqtt_reasm_single_shot_completes_immediately(void)
{
    reasm_setup();
    bool complete = bb_mqtt_client_reasm_step(&s_st, "t/single", strlen("t/single"),
                                        5, 0, "hello", 5);
...
void test_bb_mqtt_reasm_three_fragment_concat(void)
{
    ...
    bool c1 = bb_mqtt_client_reasm_step(&s_st, "t/multi", strlen("t/multi"), 9, 0, "abc", 3);
    TEST_ASSERT_FALSE(c1);
    bool c2 = bb_mqtt_client_reasm_step(&s_st, NULL, 0, 9, 3, "def", 3);
    TEST_ASSERT_FALSE(c2);
    bool c3 = bb_mqtt_client_reasm_step(&s_st, NULL, 0, 9, 6, "ghi", 3);
    TEST_ASSERT_TRUE(c3);
...
void test_bb_mqtt_reasm_total_exceeds_buffer_is_dropped(void)
{
    ...
    bool complete = bb_mqtt_client_reasm_step(&s_st, "t/big", strlen("t/big"),
                                        64, 0, "xxxxxxxx", 8);
    TEST_ASSERT_FALSE(complete);
    TEST_ASSERT_FALSE(s_st.active);

    // Trailing fragment of the same (dropped) message must also be ignored.
    bool trailing = bb_mqtt_client_reasm_step(&s_st, NULL, 0, 64, 8, "yyyyyyyy", 8);
...
{
    reasm_setup();
    bool complete = bb_mqtt_client_reasm_step(&s_st, "t/empty", strlen("t/empty"),
                                        0, 0, NULL, 0);
    TEST_ASSERT_TRUE(complete);
...
void test_bb_mqtt_reasm_mid_message_overflow_is_dropped(void)
{
    ...
    bool c1 = bb_mqtt_client_reasm_step(&s_st, "t/overflow", strlen("t/overflow"), 10, 0, "12345", 5);
    TEST_ASSERT_FALSE(c1);
    bool c2 = bb_mqtt_client_reasm_step(&s_st, NULL, 0, 10, 5, "678901234", 9); // 5+9=14 > total(10)
    TEST_ASSERT_FALSE(c2);
    TEST_ASSERT_FALSE(s_st.active);

    // A further trailing fragment for the same dropped message is ignored.
    bool c3 = bb_mqtt_client_reasm_step(&s_st, NULL, 0, 10, 14, "z", 1);
    TEST_ASSERT_FALSE(c3);
}

void test_bb_mqtt_reasm_no_buffer_is_safe_noop(void)
{
    bb_mqtt_client_reasm_state_t st;
    memset(&st, 0, sizeof(st));  // buf == NULL, buf_cap == 0
    bool complete = bb_mqtt_client_reasm_step(&st, "t", 1, 5, 0, "hello", 5);
    TEST_ASSERT_FALSE(complete);
}
...
{
    reasm_setup();
    bb_mqtt_client_reasm_step(&s_st, "t/first", strlen("t/first"), 3, 0, "abc", 3);
    bool complete = bb_mqtt_client_reasm_step(&s_st, "t/second", strlen("t/second"), 3, 0, "xyz", 3);
...
{
    bb_mqtt_client_reasm_reset(NULL);  // must not crash
}

void test_bb_mqtt_reasm_null_state_is_safe_noop(void)
{
    bool complete = bb_mqtt_client_reasm_step(NULL, "t", 1, 5, 0, "hello", 5);
    TEST_ASSERT_FALSE(complete);
}
...
{
    reasm_setup();
    bool complete = bb_mqtt_client_reasm_step(&s_st, NULL, 0, 3, 0, "abc", 3);
    TEST_ASSERT_TRUE(complete);
...
{
    reasm_setup();
    bool complete = bb_mqtt_client_reasm_step(&s_st, "t/ignored", 0, 3, 0, "abc", 3);
    TEST_ASSERT_TRUE(complete);
    TEST_ASSERT_EQUAL_STRING("", s_st.topic);
}
...
{
    reasm_setup();
    char long_topic[BB_MQTT_CLIENT_REASM_TOPIC_MAX + 32];
    memset(long_topic, 'x', sizeof(long_topic));
    long_topic[sizeof(long_topic) - 1] = '\0';

    bool complete = bb_mqtt_client_reasm_step(&s_st, long_topic, strlen(long_topic), 1, 0, "a", 1);
    TEST_ASSERT_TRUE(complete);
    TEST_ASSERT_EQUAL_INT(BB_MQTT_CLIENT_REASM_TOPIC_MAX - 1, (int)strlen(s_st.topic));
}
...
void test_bb_mqtt_reasm_buf_cap_exceeded_mid_message_is_dropped(void)
{
    ...
    // side of the OR at bb_mqtt_client_reasm_step's overflow check), independent
    // of whether it also overruns total.
    bool c1 = bb_mqtt_client_reasm_step(&s_st, "t/cap", strlen("t/cap"), TEST_BUF_CAP, 0,
                                  "0123456789", 10);
    TEST_ASSERT_FALSE(c1);
    bool c2 = bb_mqtt_client_reasm_step(&s_st, NULL, 0, TEST_BUF_CAP, 10, "0123456789", 10);
    TEST_ASSERT_FALSE(c2);
    TEST_ASSERT_FALSE(s_st.active);
}
...
{
    reasm_setup();
    bool c1 = bb_mqtt_client_reasm_step(&s_st, "t/cont", strlen("t/cont"), 10, 0, "abc", 3);
    TEST_ASSERT_FALSE(c1);
    // Non-NULL pointer, but zero-length continuation fragment.
    bool c2 = bb_mqtt_client_reasm_step(&s_st, NULL, 0, 10, 3, "", 0);
    TEST_ASSERT_FALSE(c2);
