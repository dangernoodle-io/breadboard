...
static bb_websocket_frame_type_t s_last_handler_frame_type;
                              ...
                              const bb_websocket_frame_t *frame)
{
    ...
    bb_websocket_frame_t reply = {
        .final   = true,
        .type    = frame->type,
        .payload = frame->payload,
        .len     = frame->len,
    };
    return bb_websocket_send_frame(req, &reply);
}
...
{
    ws_test_setup();
    bb_err_t err = bb_websocket_register_endpoint(NULL, "/ws", echo_handler);
...
{
    ws_test_setup();
    bb_err_t err = bb_websocket_register_endpoint(NULL, NULL, echo_handler);
...
{
    ws_test_setup();
    bb_err_t err = bb_websocket_register_endpoint(NULL, "/ws", NULL);
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_ARG, err);
}
...
{
    ws_test_setup();
    bb_websocket_frame_t frame = {0};
    bb_err_t err = bb_websocket_recv_frame(NULL, &frame, 0);
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_ARG, err);
}
...
void test_bb_websocket_recv_frame_null_frame(void)
{
    ...
    bb_err_t err = bb_websocket_recv_frame(req, NULL, 0);
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_ARG, err);
}
...
{
    ws_test_setup();
    bb_websocket_register_endpoint(NULL, "/ws", echo_handler);
    ...
    uint8_t data[] = "hello";
    bb_websocket_frame_t in = {
        .final   = true,
        .type    = BB_WS_TYPE_TEXT,
        .payload = data,
        .len     = 5,
    };
    bb_websocket_host_inject_frame(req, &in);

    // Probe call: max_len=0 returns frame len without a payload copy.
    bb_websocket_frame_t probe = {0};
    bb_err_t err = bb_websocket_recv_frame(req, &probe, 0);
    TEST_ASSERT_EQUAL(BB_OK, err);
...
void test_bb_websocket_recv_frame_force_fail(void)
{
    ...
    bb_http_request_t *req = NULL;
    bb_websocket_host_capture_begin(&req);
    bb_websocket_host_force_recv_fail(true);
    bb_websocket_frame_t frame = {0};
    bb_err_t err = bb_websocket_recv_frame(req, &frame, 0);
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_STATE, err);
    bb_websocket_host_force_recv_fail(false);
}
...
void test_bb_websocket_recv_frame_with_payload(void)
{
    ...
    uint8_t buf[16] = {0};
    bb_websocket_frame_t recv_f = {
        .payload = buf,
        .len     = 5,
    };
    bb_err_t err = bb_websocket_recv_frame(req, &recv_f, 5);
    TEST_ASSERT_EQUAL(BB_OK, err);
...
void test_bb_websocket_recv_frame_null_payload_buffer_zero_len(void)
{
    ...
    bb_websocket_frame_t recv_f = { .payload = NULL, .len = 0 };
    bb_err_t err = bb_websocket_recv_frame(req, &recv_f, 4);
    TEST_ASSERT_EQUAL(BB_OK, err);
}
...
{
    ws_test_setup();
    bb_websocket_frame_t frame = { .final = true, .type = BB_WS_TYPE_TEXT,
                                   .payload = (uint8_t*)"hi", .len = 2 };
    bb_err_t err = bb_websocket_send_frame(NULL, &frame);
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_ARG, err);
}
...
void test_bb_websocket_send_frame_null_frame(void)
{
    ...
    bb_http_request_t *req = NULL;
    bb_websocket_host_capture_begin(&req);
    bb_err_t err = bb_websocket_send_frame(req, NULL);
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_ARG, err);
}
...
void test_bb_websocket_send_frame_captures_payload(void)
{
    ...
    bb_err_t err = bb_websocket_send_frame(req, &frame);
    TEST_ASSERT_EQUAL(BB_OK, err);
...
void test_bb_websocket_send_frame_force_fail(void)
{
    ...
    bb_http_request_t *req = NULL;
    bb_websocket_host_capture_begin(&req);
    bb_websocket_host_force_send_fail(true);
    uint8_t payload[] = "x";
    bb_websocket_frame_t frame = {
        .final   = true,
        .type    = BB_WS_TYPE_TEXT,
    ...
    bb_websocket_host_force_send_fail(false);
}
...
void test_bb_websocket_capture_sent_frame_second_call_empty(void)
{
    ...
    bb_websocket_host_capture_begin(&req);
    uint8_t p[] = "a";
    bb_websocket_frame_t f = { .final=true, .type=BB_WS_TYPE_TEXT, .payload=p, .len=1 };
    bb_websocket_send_frame(req, &f);

    bb_websocket_host_capture_t cap1 = {0};
    bb_websocket_host_capture_sent_frame(&cap1);
    bb_websocket_host_capture_free(&cap1);

    // Second drain: ownership was transferred, should be empty.
    bb_websocket_host_capture_t cap2 = {0};
    bb_websocket_host_capture_sent_frame(&cap2);
...
{
    // Must not crash.
    bb_websocket_host_capture_sent_frame(NULL);
}

void test_bb_websocket_capture_free_null(void)
{
    // Must not crash.
    bb_websocket_host_capture_free(NULL);
}
...
void test_bb_websocket_inject_frame_null_req(void)
{
    ...
    bb_err_t rc = bb_websocket_host_inject_frame(NULL, &in);
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_STATE, rc);
}
...
void test_bb_websocket_inject_frame_null_frame(void)
{
    ...
    bb_http_request_t *req = NULL;
    bb_websocket_host_capture_begin(&req);
    bb_err_t rc = bb_websocket_host_inject_frame(req, NULL);
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_STATE, rc);
}
...
{
    ws_test_setup();
    bb_websocket_register_endpoint(NULL, "/ws", fail_handler);
    bb_http_request_t *req = NULL;
    bb_websocket_host_capture_begin(&req);
    uint8_t d[] = "x";
    bb_websocket_frame_t in = { .final=true, .type=BB_WS_TYPE_TEXT, .payload=d, .len=1 };
    bb_err_t rc = bb_websocket_host_inject_frame(req, &in);
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_STATE, rc);
}
...
{
    ws_test_setup();
    bb_err_t err = bb_websocket_broadcast_frame_async(NULL, 0, NULL, NULL, NULL);
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_ARG, err);
}
...
void test_bb_websocket_broadcast_frame_async_captures(void)
{
    ...
    bb_err_t err = bb_websocket_broadcast_frame_async(NULL, 5, &frame, async_cb, NULL);
    TEST_ASSERT_EQUAL(BB_OK, err);
    TEST_ASSERT_EQUAL(1, s_cb_count);
    TEST_ASSERT_EQUAL(BB_OK, s_cb_err);
    TEST_ASSERT_EQUAL(1, bb_websocket_host_async_count());
...
void test_bb_websocket_broadcast_frame_async_null_cb(void)
{
    ...
    bb_websocket_frame_t frame = { .final=true, .type=BB_WS_TYPE_TEXT,
                                   .payload=p, .len=1 };
    bb_err_t err = bb_websocket_broadcast_frame_async(NULL, 0, &frame, NULL, NULL);
    TEST_ASSERT_EQUAL(BB_OK, err);
}
...
{
    ws_test_setup();
    bb_websocket_frame_t frame = { .final=true, .type=BB_WS_TYPE_PING,
                                   .payload=NULL, .len=0 };
    bb_err_t err = bb_websocket_broadcast_frame_async(NULL, 1, &frame, NULL, NULL);
    TEST_ASSERT_EQUAL(BB_OK, err);
    const bb_websocket_host_async_capture_t *a = bb_websocket_host_async_at(0);
    TEST_ASSERT_NOT_NULL(a);
...
void test_bb_websocket_broadcast_frame_async_force_alloc_fail(void)
{
    ...
    bb_websocket_frame_t frame = { .final=true, .type=BB_WS_TYPE_TEXT,
                                   .payload=p, .len=1 };
    bb_err_t err = bb_websocket_broadcast_frame_async(NULL, 3, &frame, async_cb, NULL);
    TEST_ASSERT_EQUAL(BB_ERR_NO_SPACE, err);
    TEST_ASSERT_EQUAL(1, s_cb_count);
...
{
    ws_test_setup();
    const bb_websocket_host_async_capture_t *a = bb_websocket_host_async_at(-1);
    TEST_ASSERT_NULL(a);
    a = bb_websocket_host_async_at(100);
    TEST_ASSERT_NULL(a);
}
...
void test_bb_websocket_async_reset(void)
{
    ...
    bb_websocket_frame_t frame = { .final=true, .type=BB_WS_TYPE_TEXT,
                                   .payload=p, .len=1 };
    bb_websocket_broadcast_frame_async(NULL, 0, &frame, NULL, NULL);
    TEST_ASSERT_EQUAL(1, bb_websocket_host_async_count());
    bb_websocket_host_async_reset();
    TEST_ASSERT_EQUAL(0, bb_websocket_host_async_count());
}
...
{
    ws_test_setup();
    TEST_ASSERT_FALSE(bb_websocket_is_client(NULL, 0));
}
...
void test_bb_websocket_is_client_after_set(void)
{
    ...
    TEST_ASSERT_TRUE(bb_websocket_is_client(NULL, 2));
    bb_websocket_host_set_client_active(2, false);
    TEST_ASSERT_FALSE(bb_websocket_is_client(NULL, 2));
}

void test_bb_websocket_is_client_negative_fd(void)
{
    ws_test_setup();
    TEST_ASSERT_FALSE(bb_websocket_is_client(NULL, -1));
}

void test_bb_websocket_is_client_fd_too_large(void)
{
    ws_test_setup();
    TEST_ASSERT_FALSE(bb_websocket_is_client(NULL, BB_WEBSOCKET_MAX_FD + 10));
}
...
void test_bb_websocket_set_client_active_clear_all(void)
{
    ...
    bb_websocket_host_set_client_active(-1, false);  // fd=-1 clears all
    TEST_ASSERT_FALSE(bb_websocket_is_client(NULL, 0));
    TEST_ASSERT_FALSE(bb_websocket_is_client(NULL, 1));
}
...
void test_bb_websocket_broadcast_all_no_clients(void)
{
    ...
    bb_err_t err = bb_websocket_broadcast_all(NULL, &frame, NULL, NULL);
    TEST_ASSERT_EQUAL(BB_OK, err);
    TEST_ASSERT_EQUAL(0, bb_websocket_host_async_count());
}
...
void test_bb_websocket_broadcast_all_two_clients(void)
{
    ...
    bb_websocket_frame_t frame = { .final=true, .type=BB_WS_TYPE_TEXT,
                                   .payload=p, .len=sizeof(p)-1 };
    bb_err_t err = bb_websocket_broadcast_all(NULL, &frame, async_cb, NULL);
    TEST_ASSERT_EQUAL(BB_OK, err);
    TEST_ASSERT_EQUAL(2, bb_websocket_host_async_count());
...
{
    ws_test_setup();
    bb_err_t err = bb_websocket_broadcast_all(NULL, NULL, NULL, NULL);
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_ARG, err);
}
...
{
    ws_test_setup();
    bb_err_t err = bb_websocket_register_described_endpoint(
...
void test_bb_websocket_reset_captures_clears_state(void)
{
    ...
    TEST_ASSERT_FALSE(bb_websocket_is_client(NULL, 4));
