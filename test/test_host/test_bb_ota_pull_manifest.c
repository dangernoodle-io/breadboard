...
//
// Board name is now configured via bb_ota_check_set_firmware_board() —
...
// the board name set in each test via bb_ota_check_set_firmware_board().
...
{
    reset_world();
    bb_ota_check_set_firmware_board("myboard");
...
void test_ota_pull_manifest_fallback_board_name(void)
{
    ...
    reset_world();
    bb_ota_check_set_firmware_board(NULL);
