/* test_bb_mem_arena_tls.c — host tests for bb_mem_arena_tls routing logic.
 *
 * Built with CONFIG_BB_MEM_ARENA_TLS_BYTES=256 and BB_MEM_ARENA_TLS_TESTING so the
 * arena is enabled and the calloc/free/test_reset hooks are available.
 * 256 bytes (minus the bb_mem_arena header) gives a handful of 16-byte-aligned
...
#define ARENA_BYTES CONFIG_BB_MEM_ARENA_TLS_BYTES

void test_bb_mem_arena_tls_init_is_idempotent(void)
{
    bb_mem_arena_tls_test_reset();
    bb_mem_arena_tls_init();
    bb_mem_arena_tls_init(); /* second call must not crash */
}
...
void test_bb_mem_arena_tls_alloc_hit_returns_in_arena(void)
{
    bb_mem_arena_tls_test_reset();
    bb_mem_arena_tls_init();
    void *p = bb_mem_arena_tls_calloc(1, 16);
    TEST_ASSERT_NOT_NULL(p);
    TEST_ASSERT_TRUE(bb_mem_arena_tls_owns(p));
    bb_mem_arena_tls_free(p);
}
...
void test_bb_mem_arena_tls_alloc_is_zeroed(void)
{
    bb_mem_arena_tls_test_reset();
    bb_mem_arena_tls_init();
    uint8_t *p = bb_mem_arena_tls_calloc(1, 32);
    TEST_ASSERT_NOT_NULL(p);
    for (int i = 0; i < 32; i++) {
...
void test_bb_mem_arena_tls_exhausted_falls_back_to_heap(void)
{
    ...
    for (int i = 0; i < 64; i++) {
        ptrs[i] = bb_mem_arena_tls_calloc(1, 16);
        if (ptrs[i] == NULL) break;
        n++;
        if (!bb_mem_arena_tls_owns(ptrs[i])) {
            got_heap_ptr = true;
            break;
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        if (ptrs[i]) bb_mem_arena_tls_free(ptrs[i]);
    }
}
...
void test_bb_mem_arena_tls_free_in_range_routes_to_arena(void)
{
    bb_mem_arena_tls_test_reset();
    bb_mem_arena_tls_init();
    void *p = bb_mem_arena_tls_calloc(1, 8);
    TEST_ASSERT_NOT_NULL(p);
    TEST_ASSERT_TRUE(bb_mem_arena_tls_owns(p));
    bb_mem_arena_tls_free(p); /* must not crash — routes to arena path */
}
...
void test_bb_mem_arena_tls_free_out_of_range_routes_to_heap(void)
{
    ...
    void *p = malloc(8);
    TEST_ASSERT_NOT_NULL(p);
    TEST_ASSERT_FALSE(bb_mem_arena_tls_owns(p));
    bb_mem_arena_tls_free(p); /* must not crash — routes to free() */
}

void test_bb_mem_arena_tls_free_null_is_noop(void)
{
    bb_mem_arena_tls_free(NULL); /* must not crash */
}

void test_bb_mem_arena_tls_owns_null_is_false(void)
{
    TEST_ASSERT_FALSE(bb_mem_arena_tls_owns(NULL));
}
...
void test_bb_mem_arena_tls_calloc_zero_returns_null(void)
{
    bb_mem_arena_tls_test_reset();
    bb_mem_arena_tls_init();
    void *p = bb_mem_arena_tls_calloc(0, 16);
...
void test_bb_mem_arena_tls_reuse_after_free_all(void)
{
    ...
    for (int i = 0; i < 64; i++) {
        void *p = bb_mem_arena_tls_calloc(1, 16);
        if (p == NULL || !bb_mem_arena_tls_owns(p)) break;
        ptrs[n++] = p;
    }
    ...
    void *p2 = bb_mem_arena_tls_calloc(1, 16);
    TEST_ASSERT_NOT_NULL(p2);
    TEST_ASSERT_TRUE(bb_mem_arena_tls_owns(p2));
    bb_mem_arena_tls_free(p2);
}
...
void test_bb_mem_arena_tls_double_free_does_not_underflow(void)
{
    ...
    bb_mem_arena_tls_free(p);
    bb_mem_arena_tls_free(p); /* double-free: must not crash or underflow */
