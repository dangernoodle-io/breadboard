// Tests for bb_mqtt_on_message / bb_mqtt_host_inject_message (B1-487): the
...
static bb_mqtt_t make_handle(void)
{
...
{
    reset_capture();
    bb_mqtt_t h = make_handle();
    TEST_ASSERT_EQUAL(BB_OK, bb_mqtt_on_message(h, capture_cb, NULL));

    bb_mqtt_host_inject_message(h, "test/topic", "hello", 5);
...
void test_bb_mqtt_on_message_passes_ctx(void)
{
    ...
    bb_mqtt_t h = make_handle();
    int marker = 42;
    TEST_ASSERT_EQUAL(BB_OK, bb_mqtt_on_message(h, capture_cb, &marker));

    bb_mqtt_host_inject_message(h, "t", "v", 1);
...
void test_bb_mqtt_on_message_multiple_injections(void)
{
    reset_capture();
    bb_mqtt_t h = make_handle();
    bb_mqtt_on_message(h, capture_cb, NULL);

    bb_mqtt_host_inject_message(h, "a", "1", 1);
    bb_mqtt_host_inject_message(h, "b", "22", 2);
...
void test_bb_mqtt_on_message_replaces_previous_callback(void)
{
    ...
    bb_mqtt_t h = make_handle();
    bb_mqtt_on_message(h, capture_cb, NULL);
    bb_mqtt_host_inject_message(h, "first", "1", 1);
    TEST_ASSERT_EQUAL_INT(1, s_calls);

    // Registering NULL clears the slot — subsequent injects are no-ops.
    bb_mqtt_on_message(h, NULL, NULL);
    bb_mqtt_host_inject_message(h, "second", "2", 1);
...
{
    TEST_ASSERT_EQUAL(BB_ERR_INVALID_ARG, bb_mqtt_on_message(NULL, capture_cb, NULL));
}
...
{
    bb_mqtt_t h1 = make_handle();
    bb_mqtt_t h2 = make_handle();

    handle_capture_t ctx1 = {0};
    handle_capture_t ctx2 = {0};
    TEST_ASSERT_EQUAL(BB_OK, bb_mqtt_on_message(h1, handle_capture_cb, &ctx1));
    TEST_ASSERT_EQUAL(BB_OK, bb_mqtt_on_message(h2, handle_capture_cb, &ctx2));
    ...
    bb_mqtt_host_inject_fragment(h1, "a/x", 9, 0, "AAA", 3);
    bb_mqtt_host_inject_fragment(h1, "a/x", 9, 3, "BBB", 3);
    ...
    bb_mqtt_host_inject_message(h2, "b/y", "zzzzz", 5);
    ...
    bb_mqtt_host_inject_fragment(h1, "a/x", 9, 6, "CCC", 3);
