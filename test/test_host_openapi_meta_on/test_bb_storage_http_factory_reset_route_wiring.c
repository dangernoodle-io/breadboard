...
// EMITS for the real production POST /api/diag/factory-reset route after a
...
// (the pre-fix bug: s_factory_reset_route.request_schema pointed DIRECTLY
// at s_factory_reset_request_schema_buf, a link-time-constant address
// that's always non-NULL even when that buffer is left empty by a failed
// compose). Drives the REAL production init fn
// (bb_storage_http_factory_reset_routes_init(), not a hand-built twin) with
// a dummy handle, so this exercises production's own s_factory_reset_route,
// not a copy. MUST run before the success test right below (compose is
// guarded/idempotent -- a non-empty buffer short-circuits a second real
// assemble once one has succeeded).
void test_bb_storage_http_factory_reset_route_omits_request_body_on_compose_failure(void)
{
    bb_http_route_registry_clear();
    bb_data_test_reset();
    ...
    cJSON *factory_reset_path = cJSON_GetObjectItemCaseSensitive(
        cJSON_GetObjectItemCaseSensitive(doc, "paths"), "/api/diag/factory-reset");
    TEST_ASSERT_NOT_NULL(factory_reset_path);
    cJSON *factory_reset_post = cJSON_GetObjectItemCaseSensitive(factory_reset_path, "post");
    TEST_ASSERT_NOT_NULL(factory_reset_post);
    TEST_ASSERT_NULL(cJSON_GetObjectItemCaseSensitive(factory_reset_post, "requestBody"));
