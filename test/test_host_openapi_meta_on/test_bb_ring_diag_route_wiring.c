...
// proves bb_ring_diag's runtime-compose path (B1-1059 PR-3 batch 1)
...
// pointer-stable (idempotent -- never re-assembles once patched). The
...
// (bb_ring_diag.c) -- a distinct branch from the helper's own error arm,
// which only the helper's dedicated test covers.

// Exercises the fail-loud `if (rc != BB_OK) return rc;` branch inside
// ensure_schema_patched() (bb_ring_diag.c) -- forces the engine
...
void test_bb_ring_diag_assemble_schema_offline_on_compose_failure(void)
{
    ...
    bb_err_t rc = bb_ring_diag_assemble_schema_for_test();

    TEST_ASSERT_EQUAL(BB_ERR_NO_SPACE, rc);
    TEST_ASSERT_NULL(bb_ring_diag_get_describe_schema_for_test());
...
void test_bb_ring_diag_assemble_schema_patches_matching_content(void)
{
    ...
    const char *schema = bb_ring_diag_get_describe_schema_for_test();
    TEST_ASSERT_NOT_NULL(schema);
    TEST_ASSERT_EQUAL_STRING(bb_ring_diag_schema, schema);
}

void test_bb_ring_diag_assemble_schema_idempotent_pointer_stable(void)
{
    TEST_ASSERT_EQUAL(BB_OK, bb_ring_diag_assemble_schema_for_test());
    const char *first = bb_ring_diag_get_describe_schema_for_test();

    TEST_ASSERT_EQUAL(BB_OK, bb_ring_diag_assemble_schema_for_test());
    const char *second = bb_ring_diag_get_describe_schema_for_test();
