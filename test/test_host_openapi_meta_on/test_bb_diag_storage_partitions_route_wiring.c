...
void test_bb_diag_storage_partitions_assemble_schema_offline_on_compose_failure(void)
{
    ...
    bb_err_t rc = bb_diag_storage_partitions_assemble_schema_for_test();
...
void test_bb_diag_storage_partitions_assemble_schema_patches_matching_content(void)
{
    ...
    const char *schema = bb_diag_storage_partitions_get_describe_schema_for_test();
    TEST_ASSERT_NOT_NULL(schema);
    TEST_ASSERT_EQUAL_STRING(bb_diag_storage_partitions_schema, schema);
}

void test_bb_diag_storage_partitions_assemble_schema_idempotent_pointer_stable(void)
{
    TEST_ASSERT_EQUAL(BB_OK, bb_diag_storage_partitions_assemble_schema_for_test());
    const char *first = bb_diag_storage_partitions_get_describe_schema_for_test();

    TEST_ASSERT_EQUAL(BB_OK, bb_diag_storage_partitions_assemble_schema_for_test());
    const char *second = bb_diag_storage_partitions_get_describe_schema_for_test();
