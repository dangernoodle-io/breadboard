...
// composed()'s own test (B1-1204); the compose-failure test below is NEW
// (B1-1204 review fix -- the pilot never had one) because it exercises
// THIS site's own `if (rc != BB_OK) return rc;` propagation arm in
// ensure_schema_patched() (bb_diag_storage_nvs.c) -- a distinct,
// genuinely pre-existing, previously-untested branch from the helper's
// own error arm, which only the helper's dedicated test covers.

// Exercises the fail-loud `if (rc != BB_OK) return rc;` branch inside
// ensure_schema_patched() (bb_diag_storage_nvs.c) -- forces the engine
...
void test_bb_diag_storage_nvs_assemble_schema_offline_on_compose_failure(void)
{
    ...
    bb_err_t rc = bb_diag_storage_nvs_assemble_schema_for_test();
