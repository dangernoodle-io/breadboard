...
static bb_err_t wsbcast_handler(bb_http_request_t *req)
{
    ...
    bb_websocket_broadcast_all(bb_http_server_get_handle(), &frame, NULL, NULL);
    return bb_http_resp_no_content(req);
}
...
void smoke_app_setup(void) {
    ...
    // bb_websocket: /ws echo + /api/wsbcast broadcast demo.
    {
        ...
        bb_err_t wserr = bb_websocket_register_described_endpoint(
            svr, "/ws", ws_echo_handler, &s_ws_route);
        if (wserr == BB_OK) {
            bb_log_i(TAG, "bb_websocket: /ws registered");
        } else {
            bb_log_w(TAG, "bb_websocket: /ws register failed (%d)", (int)wserr);
        }
        if (bb_http_register_route(svr, BB_HTTP_POST, "/api/wsbcast",
                                   wsbcast_handler) == BB_OK) {
            bb_log_i(TAG, "bb_websocket: /api/wsbcast registered");
        }
    }
