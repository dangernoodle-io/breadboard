...
// B1-1425 / B1-1045 PR-4 precedent: bb_data_gather_plain()'s ctx wrapping the
// dissolved-bb_event "log" producer's plain gather -- portable/host-testable
// at the producer's own layer (see bb_log_event_gather()'s doc comment);
// this composition root just supplies the typed fill pointer, the shared
// thunk supplies the untyped bb_data_gather_fn cast. Mirrors
// examples/floor/main/floor_app.c's own s_log_fill_ctx exactly, but not
// static -- see smoke_ping_handler's comment above; examples/smoke/main/
// bb_wire.h's bb_data_bind() manifest entry references this by address.
const bb_data_plain_fill_ctx_t smoke_log_fill_ctx = {
