...
coverage: test ## Coverage report (gcovr merges .gcda across every env `test` above just built -- including native_openapi_runtime_meta, B1-1093, so non-default-Kconfig-gated arms are measured, not silently compiled-out-and-invisible), gated on the committed coverage baseline (B1-764 shrink-only per-file ratchet, now covering BOTH per-LINE and per-line-BRANCH-existence markers -- B1-1258, see scripts/coverage_baseline.py); verified-toolchain-gated (B1-642 matched-major + B1-867 genuinely-GNU gcc/gcov enforced -- no specific major version required; aborts loudly instead of silently reporting 0%)
