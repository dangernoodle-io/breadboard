...
test: ## Run host unit tests (both compile-time BB_LOCK_STATS_ENABLE states, the CONFIG_BB_OPENAPI_RUNTIME_META=1 wiring env, plus the CONFIG_BB_WIFI_PROV_GATE_DISABLE=1 escape-hatch env) under a verified, matched, genuinely-GNU gcc/gcov toolchain -- any major version (B1-642 -- PlatformIO strips CC/CXX, so scripts/coverage_toolchain.sh PATH-shims instead). Verdict comes from a direct run of the compiled binary, not pio's own test reader (B1-1137 -- see scripts/run_host_tests.py)
	./scripts/coverage_toolchain.sh python3 scripts/run_host_tests.py --pio '$(PIO)' native native_lock_stats_off native_openapi_runtime_meta native_prov_gate_disable

coverage: test ## Coverage report (gcovr merges .gcda across every env `test` above just built -- including native_openapi_runtime_meta, B1-1093, so non-default-Kconfig-gated arms are measured, not silently compiled-out-and-invisible), gated on the committed coverage baseline (B1-764 shrink-only per-file ratchet). LOCALLY this ratchets BOTH per-LINE and per-line-BRANCH-existence markers; in CI it ratchets LINES ONLY (COVERAGE_GATE_LINES_ONLY, set by .github/workflows/build.yml's pre-build-script) because branch attribution is gcc-major-dependent and the branch baseline is seeded on a dev machine -- Coveralls remains the CI authority for branch coverage (B1-1258, see scripts/coverage_baseline.py); verified-toolchain-gated (B1-642 matched-major + B1-867 genuinely-GNU gcc/gcov enforced -- no specific major version required; aborts loudly instead of silently reporting 0%)
