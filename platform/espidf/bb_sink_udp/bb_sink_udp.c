// bb_sink_udp — ESP-IDF backend: real IPv4 UDP socket transport
// (ouroboros KB#554). One AF_INET/SOCK_DGRAM socket, lazily opened on first
// publish and kept open for the process lifetime. Unicast sends target
// cfg.host:cfg.port (literal IPv4, no DNS resolution); broadcast mode sends
// to 255.255.255.255:cfg.port after arming SO_BROADCAST once. The
// destination sockaddr_in is rebuilt from the current cfg on every send so a
// runtime cfg change takes effect immediately — the socket itself is never
// connect()ed. Same-subnet delivery needs no gateway hop.
...
static const char *TAG = "bb_sink_udp";
...
// because init runs exactly once at PRE_HTTP boot, strictly before bb_pub
// wires up publish; and publish() (udp_publish, below) runs only on the
// single bb_pub worker task. There is no runtime config setter that could
// mutate s_cfg concurrently with a publish — so no lock is needed. If a
// runtime reconfigure API is ever added, this invariant must be revisited.
                             ...
                             const char *payload, int len, bool retain)
{
    ...
    if (n < 0) {
        bb_log_w(TAG, "publish: frame exceeds MTU %d, dropped", BB_SINK_UDP_MTU);
    ...
    if (s_cfg.broadcast) {
        ...
        if (inet_pton(AF_INET, s_cfg.host, &dest.sin_addr) != 1) {
            bb_log_e(TAG, "publish: invalid host configured");
            return BB_ERR_INVALID_ARG;
        }
    }

    ssize_t sent = sendto(s_sock, buf, (size_t)n, 0,
...
{
    bb_err_t err = bb_sink_udp_init(NULL);
