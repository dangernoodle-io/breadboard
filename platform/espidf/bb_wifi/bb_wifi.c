...
// Got-IP callback. NOTE: stays hand-rolled (not the bb_core
// BB_CALLBACK_SLOT_VOID0 macro) -- bb_wifi_register_on_got_ip has extra
// late-registration REPLAY logic (fires immediately if s_has_ip is already
// set) that a plain slot+invoke doesn't cover. A future macro variant may
// add replay support; until then this seam is grandfathered from the
// consolidation rule/fence.
static bb_wifi_on_got_ip_cb_t s_on_got_ip_cb = NULL;

// Disconnect callback: slot + setter (bb_wifi_register_on_disconnect) + the
// public invoke (bb_wifi_on_disconnect_invoke, declared in wifi_reconn.h)
// are generated by the bb_core BB_CALLBACK_SLOT_VOID0 macro, instantiated in
// platform/host/bb_wifi/bb_wifi_emit.c (host+esp compiled) -- see the
// ota-validated seam's comment above for the same pattern.

// Net-event sink (KB 781 PR4-core, bb_wifi_set_net_event_sink): slot +
// setter + the public invoke (bb_wifi_net_event_invoke, declared in
// wifi_reconn.h) are generated by the bb_core BB_CALLBACK_SLOT_VOID macro
// (with-args variant), instantiated in platform/host/bb_wifi/bb_wifi_emit.c
// (host+esp compiled) -- fired at the three STA lifecycle edges below
// (got_ip/disconnect/lost_ip). NULL sink (default) = no-op.
