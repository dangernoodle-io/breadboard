...
// every bb_queue_create/destroy/count call fleet-wide. Instead: copy each
...
// from that snapshot AFTER bb_queue_registry_foreach returns (lock released).
...
static void rings_snapshot_cb(const char *name, bb_queue_t r, void *ctx)
{
    ...
    if (sc->n >= sc->max) {
        return;  // registry is bounded by BB_QUEUE_REGISTRY_MAX; defensive only
    }
    ...
    // counts/dropped/truncated (bounded by BB_QUEUE_REGISTRY_MAX-sized rings)
    // and cumulative byte totals — none realistically approach 4G on an
    // embedded target, so truncation is not a practical concern.
    s->count      = (uint32_t)bb_queue_count(r);
    s->capacity   = (uint32_t)bb_queue_capacity(r);
    s->dropped    = (uint32_t)bb_queue_dropped(r);
    s->truncated  = (uint32_t)bb_queue_truncated(r);
    s->bytes_used = (uint32_t)bb_queue_bytes_used(r);
    sc->n++;
}

static bb_err_t rings_get_handler(bb_http_request_t *req)
{
    // Snapshot phase — bb_queue_registry's lock is held for the duration of
    ...
    ring_snapshot_t snap[CONFIG_BB_QUEUE_REGISTRY_MAX];
    snapshot_ctx_t sc = { .snap = snap, .n = 0, .max = CONFIG_BB_QUEUE_REGISTRY_MAX };
    ...
    // bb_queue_registry lock held.
    bb_http_json_obj_stream_t obj;
    bb_err_t err = bb_http_resp_json_obj_begin(req, &obj);
    if (err != BB_OK) return err;

    bb_http_resp_json_obj_set_int(&obj, "count",             (int64_t)sc.n);
    bb_http_resp_json_obj_set_int(&obj, "registry_capacity", (int64_t)CONFIG_BB_QUEUE_REGISTRY_MAX);
...
static const bb_route_response_t s_rings_get_responses[] = {
    { 200, "application/json",
      ...
      "every live bb_queue_t registered via bb_queue_registry" },
    { 0 },
};
...
static const bb_route_t s_rings_get_route = {
    ...
    .path      = "/api/diag/rings",
    .tag       = "diag",
    .summary   = "List every live bb_queue_t (name, count, capacity, dropped, truncated, bytes_used)",
