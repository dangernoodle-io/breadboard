// ESP-IDF route handler for bb_websocket — registers GET /api/diag/websocket.
...
// CONFIG_HTTPD_WS_SUPPORT — bb_websocket_open_count() returns 0 when WS
...
static bb_err_t ws_diag_handler(bb_http_request_t *req)
{
    ...
    bb_http_resp_json_obj_set_int(&obj, "open_connections",
                                   (int64_t)bb_websocket_open_count());
...
static bb_err_t bb_websocket_diag_init(bb_http_handle_t server)
{
...
#if CONFIG_BB_WEBSOCKET_DIAG_AUTOREGISTER
BB_INIT_REGISTER(bb_websocket_diag, bb_websocket_diag_init);
#endif
