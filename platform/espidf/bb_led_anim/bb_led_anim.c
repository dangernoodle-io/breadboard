// bb_led_anim — pattern/animation library (ESP-IDF platform).
// Kept in sync with platform/host/bb_led_anim/bb_led_anim.c; differs only in
// the now_ms() implementation and the optional esp_timer auto-start path.
...
// ---------------------------------------------------------------------------
// Helpers: wall-clock, sin LUT, HSV→RGB
// ---------------------------------------------------------------------------
...
#else
    // Fallback for when this file is compiled without ESP-IDF (shouldn't happen
    // in normal builds — use the host file instead).
    return 0;
#endif
}

// 256-entry sin LUT: sin8(i) = 128 + 127*sin(2*pi*i/256), values in [0..255].
                                            ...
                                            uint16_t led_count)
{
    ...
    case BB_ANIM_CHASE:
        return (led_count > 1) ? BB_LED_CAP_RGB : (bb_led_caps_t)0xFF;
    }
