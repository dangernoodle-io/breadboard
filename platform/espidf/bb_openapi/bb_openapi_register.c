...
static bb_err_t openapi_handler(bb_http_request_t *req)
{
    ...
    // grew past ~50 — see B1-222. bb_openapi_emit_stream() owns the full
    // response lifecycle (sets Content-Type and opens the chunked response,
    // always flushes + sends the terminator on return, even on error) — no
    // separate finalize step needed here.
    return bb_openapi_emit_stream(req, &effective);
}
