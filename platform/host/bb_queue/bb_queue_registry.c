// bb_queue_registry — thin consumer of the generic bb_registry primitive.
// Compiled on both host (tests) and ESP-IDF as part of the bb_queue component.
...
BB_REGISTRY_DEFINE_TAGGED(s_ring_registry, BB_QUEUE_REGISTRY_MAX, "rings");
...
//   2. diag-read UAF — bb_queue_destroy() deregisters then frees the ring.
//      A concurrent /api/diag/rings read iterating via foreach could still
//      be dereferencing the ring's fields (bb_queue_count/dropped/...) after
...
//      bb_queue_registry_deregister() blocks on this same lock until the
//      read finishes, so bb_queue_destroy() cannot free the ring out from
//      under a concurrent diag read. This requires bb_queue_destroy() to
//      deregister BEFORE freeing r (it does — see bb_queue.c).
//
...
//
// Foreach contract (see bb_queue_registry.h): the caller's callback runs
// while s_ring_reg_lock is held, so it MUST be bounded, allocation-free,
// non-blocking, and MUST NOT call back into any bb_queue_registry_*
...
// find_by_value scan state — used by bb_queue_registry_deregister below.
typedef struct {
    void       *target;
...
bb_err_t bb_queue_registry_deregister(bb_queue_t r)
{
    ...
    // least invasive: no bb_queue.h dependency, no extra field on bb_queue_t).
...
// to bb_queue_registry_foreach's (name, bb_queue_t, ctx) shape without an
// incompatible function-pointer cast.
typedef struct {
    bb_queue_registry_cb_t cb;
    void                 *ctx;
} foreach_bridge_t;
...
static void foreach_trampoline(const char *name, void *value, void *ctx)
{
    ...
    bridge->cb(name, (bb_queue_t)value, bridge->ctx);
}
...
void bb_queue_registry_foreach(bb_queue_registry_cb_t cb, void *ctx)
{
    ...
    // above. bb_queue_destroy() cannot proceed past its deregister call
