#pragma once
// Host-only capture API for bb_ws_server — mirrors bb_http_host.h.
// Used by native tests to exercise all bb_ws_server branches without an
// actual HTTP server or WebSocket connection.
//
// Usage:
//   // Register first (captures the handler fn internally):
//   bb_ws_server_register_endpoint(NULL, "/ws", my_handler);
//
//   // Inject an incoming frame and capture any reply:
//   bb_ws_server_host_capture_t cap = {0};
//   bb_http_request_t *req = NULL;
//   bb_ws_server_host_capture_begin(&req);
//   bb_ws_server_frame_t in_frame = { .final=true, .type=BB_WS_TYPE_TEXT,
//                                     .payload=(uint8_t*)"hi", .len=2 };
//   bb_ws_server_host_inject_frame(req, &in_frame);
//   bb_ws_server_host_capture_sent_frame(&cap);
//   // inspect cap.frame, cap.err
//   bb_ws_server_host_capture_free(&cap);

#include "bb_ws_server.h"
#include "bb_http.h"

#include <stddef.h>
#include <stdint.h>
#include <stdbool.h>

#ifdef __cplusplus
extern "C" {
#endif

// Captured outgoing frame.
typedef struct {
    bb_ws_server_frame_type_t type;
    bool                      final;
    uint8_t                  *payload;  // heap copy; free via bb_ws_server_host_capture_free
    size_t                    len;
    bb_err_t                  err;      // result of the send_frame call
} bb_ws_server_host_capture_t;

// ----- Registration -------------------------------------------------------

// bb_ws_server_register_endpoint on host is a no-op for the httpd side;
// it stores the handler internally so inject_frame can call it.

// ----- Request cookie -----------------------------------------------------

// Arm a new fake request slot.  Writes a non-NULL stable pointer to *out_req.
// Call before bb_ws_server_host_inject_frame.
void bb_ws_server_host_capture_begin(bb_http_request_t **out_req);

// ----- Frame injection ----------------------------------------------------

// Deliver in_frame to the registered handler and capture any reply written
// via bb_ws_server_send_frame.  req must be the pointer from capture_begin.
// Returns the handler's return value.
bb_err_t bb_ws_server_host_inject_frame(bb_http_request_t *req,
                                        const bb_ws_server_frame_t *in_frame);

// ----- Sent-frame capture -------------------------------------------------

// Retrieve the last frame passed to bb_ws_server_send_frame since the last
// capture_begin.  Ownership of out->payload is transferred to the caller;
// call bb_ws_server_host_capture_free to release it.
void bb_ws_server_host_capture_sent_frame(bb_ws_server_host_capture_t *out);

// Free payload owned by a capture.  Safe to call with NULL payload.
void bb_ws_server_host_capture_free(bb_ws_server_host_capture_t *cap);

// ----- Reset --------------------------------------------------------------

// Clear all capture state (registered handler, pending frames, sent frame).
// Call in teardown between tests.
void bb_ws_server_host_reset_captures(void);

// ----- Async send capture -------------------------------------------------

typedef struct {
    bb_ws_server_frame_type_t type;
    bool                      final;
    uint8_t                  *payload;   // heap copy
    size_t                    len;
    int                       fd;
    bb_err_t                  err;
} bb_ws_server_host_async_capture_t;

// Maximum number of async sends captured per test.
#define BB_WS_SERVER_HOST_ASYNC_CAP 8

// Retrieve the number of async sends captured since last reset.
int bb_ws_server_host_async_count(void);

// Retrieve async capture at index i (0-based).  Returns NULL if out of range.
const bb_ws_server_host_async_capture_t *bb_ws_server_host_async_at(int i);

// Free all async captures.
void bb_ws_server_host_async_reset(void);

// ----- Client-active stub -------------------------------------------------

// Set whether fd is reported as an active WS client (used by is_client and
// broadcast_all to iterate).  Pass fd=-1 to clear all.
void bb_ws_server_host_set_client_active(int fd, bool active);

// Set the fd that bb_ws_server_req_fd() returns during bb_ws_server_host_inject_frame.
// Call before inject_frame to simulate a per-connection subscription update.
// Pass -1 to clear (default).
void bb_ws_server_host_set_inject_fd(int fd);

// ----- Force-fail hooks ---------------------------------------------------

// Force bb_ws_server_register_endpoint to return BB_ERR_INVALID_STATE.
void bb_ws_server_host_force_register_fail(bool fail);

// Force bb_ws_server_recv_frame to return BB_ERR_INVALID_STATE.
void bb_ws_server_host_force_recv_fail(bool fail);

// Force bb_ws_server_send_frame to return BB_ERR_NO_SPACE.
void bb_ws_server_host_force_send_fail(bool fail);

// Force bb_ws_server_broadcast_frame_async malloc to fail.
void bb_ws_server_host_force_async_alloc_fail(bool fail);

// ----- Open-connection counter stub ---------------------------------------

// Simulate a WS session opening: bb_ws_server_open_count() increments by 1.
void bb_ws_server_host_simulate_open(void);

// Simulate a WS session closing: bb_ws_server_open_count() decrements by 1
// (clamped at 0; a spurious close never underflows).
void bb_ws_server_host_simulate_close(void);

// ----- Disconnect callback stub -------------------------------------------

// Invoke the callback registered via bb_ws_server_set_disconnect_cb() (if
// any) with the given fd, simulating a WS session teardown notification.
void bb_ws_server_host_simulate_disconnect(int fd);

// ----- Connect callback stub -----------------------------------------------

// Invoke the callback registered via bb_ws_server_set_connect_cb() (if any)
// with the given server/fd, simulating a WS handshake-completion notification.
void bb_ws_server_host_simulate_connect(bb_http_handle_t server, int fd);

#ifdef __cplusplus
}
#endif
