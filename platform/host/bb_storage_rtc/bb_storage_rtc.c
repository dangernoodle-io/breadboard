...
// Plain static stands in for ESP-IDF's RTC_NOINIT_ATTR region on host: it
// survives across bb_storage_rtc_register() calls within one test process
// (mirroring "warm reboot"), but bb_storage_rtc_test_reset() simulates a
// full power cycle by zeroing it (mirroring what a real RTC_NOINIT region
// loses on power loss). Own, SEPARATE region from bb_nv's host stub --
// see bb_storage_rtc.h's file header for the PR3a additive-only note.
static bb_storage_rtc_region_t s_region;
...
static bb_err_t rtc_get(void *impl, const bb_storage_addr_t *addr, void *buf, size_t cap, size_t *out_len)
{
    ...
    if (!bb_storage_rtc_region_valid(&s_region)) {
        ...
        // reads as absent, regardless of stale bytes still in the region.
