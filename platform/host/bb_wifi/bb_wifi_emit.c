...
// the buffer would leak that sentinel; blank ip on any non-BB_OK return.
void bb_wifi_publish_net_event(bb_wifi_net_event_t evt, bb_wifi_disc_reason_t reason)
{
    ...
    bb_wifi_emit_invoke(BB_WIFI_EVENT_TOPIC, (int32_t)evt, &payload, sizeof(payload));
}

// One-shot current-state synthesis onto the generic emit slot (bb_wifi.h
// bb_wifi_emit_baseline). No-op if the slot is unset -- checked directly
// against this TU's own slot (bb_callback_slot_emit, generated by the
// BB_CALLBACK_SLOT_VOID instantiation above) rather than relying on
// bb_wifi_publish_net_event's own null-safe invoke, so a build with no
// generic-seam consumer never pays for bb_wifi_get_info().
void bb_wifi_emit_baseline(void)
{
    if (!bb_callback_slot_emit) return;
    bb_wifi_info_t info;
    if (bb_wifi_get_info(&info) != BB_OK) return;
    if (info.connected) {
        bb_wifi_publish_net_event(BB_WIFI_NET_EVT_GOT_IP, BB_WIFI_DISC_UNKNOWN);
    } else {
        bb_wifi_publish_net_event(BB_WIFI_NET_EVT_DISCONNECT, info.disc_reason);
    }
}
