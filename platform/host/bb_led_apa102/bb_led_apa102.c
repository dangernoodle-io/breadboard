...
static bb_err_t do_flush(state_t *s) {
    ...
    for (uint16_t i = 0; i < s->led_count; i++) {
        if (!s->enabled[i]) {
            tx_byte(slot, 0xE0);
            tx_byte(slot, 0); tx_byte(slot, 0); tx_byte(slot, 0);
            continue;
        }
        uint8_t r = s->rgb[i*3 + 0], g = s->rgb[i*3 + 1], b = s->rgb[i*3 + 2];
        uint8_t bri = s->bri[i];
        if (s->level_set[i]) {
            // Fine path: keep global current high, scale the base color by the
            // CIE-gamma envelope at 8-bit (smoother dim than the 5-bit global).
...
bb_err_t bb_led_apa102_open(const bb_led_apa102_cfg_t *cfg, bb_led_handle_t *out) {
    ...
    *drv = (bb_led_driver_t){
    ...
    if (rc != BB_OK) { free(s->rgb); free(s->bri); free(s->enabled); free(s->level16); free(s->level_set); free(drv); free(s); s_slots[slot].active = false; }
    return rc;
}
