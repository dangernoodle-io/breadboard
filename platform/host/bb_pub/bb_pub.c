...
// CONFIG_BB_PUB_MAX_SOURCES and CONFIG_BB_PUB_TOPIC_PREFIX are provided by
...
static bb_pub_sink_t   s_sink;
static bool            s_sink_set       = false;
...
bb_err_t bb_pub_tick_once(void)
{
    ...
    for (int i = 0; i < s_source_count; i++) {
        ...
        bb_err_t err = s_sink.publish(s_sink.ctx, topic, json, (int)strlen(json));
        if (err != BB_OK) {
            bb_log_w(TAG, "sink publish failed for '%s': %d", src->subtopic, err);
        }
