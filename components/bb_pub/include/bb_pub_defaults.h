...
// satellite (bb_pub_info, bb_pub_telemetry) that sizes a static snapshot
// buffer against it. On ESP-IDF the build system always supplies
// CONFIG_BB_PUB_TELEM_SNAP_MAX via sdkconfig.h, so this fallback only takes
// effect on host builds without a Kconfig.
//
