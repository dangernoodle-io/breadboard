...
//   - cap >= required                         -> FULL     (read straight into caller's buf)
//   - 0 < cap < required && required <= max   -> BOUNCE   (stage in scratch, copy cap bytes)
//   - 0 < cap < required && required > max    -> NO_SPACE (cannot safely stage)
//
...
bb_storage_nvs_get_outcome_t bb_storage_nvs_get_decide(size_t required, size_t cap,
                                                        size_t scratch_max, size_t *out_len);
