...
// CONFIG_BB_OPENAPI_RUNTIME_META (B1-1059 PR-2 pilot) -- gated DIRECTLY on
...
// the `#else` arm below is byte-identical to the pre-pilot table.
#if defined(CONFIG_BB_OPENAPI_RUNTIME_META)

// Sized to the golden-tested hand literal (B1-1180 PR-1's
// test_bb_diag_storage_nvs_meta_golden.c proves the engine's output is
// byte-identical to BB_DIAG_STORAGE_NVS_SCHEMA_LITERAL) -- not an arbitrary
...
// truncating.
static char s_diag_storage_nvs_schema_buf[sizeof(BB_DIAG_STORAGE_NVS_SCHEMA_LITERAL)];

static bb_err_t assemble_schema(void)
{
    size_t n = 0;
    return bb_serialize_meta_openapi_schema(&bb_diag_storage_nvs_desc, &bb_diag_storage_nvs_meta,
                                             s_diag_storage_nvs_schema_buf,
                                             sizeof(s_diag_storage_nvs_schema_buf), &n);
}

// Mutable (`.data`, not `.rodata`) with this config on -- `.schema` starts
// NULL and is patched in once by bb_diag_storage_nvs_register() (or the
// test accessor below) before the route is ever registered/served.
static bb_route_response_t s_diag_storage_nvs_describe_responses[] = {
...
// Shared compose-and-patch step, called from both bb_diag_storage_nvs_
...
{
    if (!s_diag_storage_nvs_describe_responses[0].schema) {
        bb_err_t rc = assemble_schema();
        if (rc != BB_OK) return rc;  // fail loud -- never register a NULL/partial schema
        s_diag_storage_nvs_describe_responses[0].schema = s_diag_storage_nvs_schema_buf;
    }
    return BB_OK;
}
#endif /* CONFIG_BB_OPENAPI_RUNTIME_META */
...
// ---------------------------------------------------------------------------
// Test-only accessors (BB_DIAG_STORAGE_NVS_TESTING) -- see
// bb_diag_storage_nvs_test.h. Portable (not ESP_PLATFORM-gated): exercises
// the same guarded, idempotent compose-and-patch bb_diag_storage_nvs_
// register() runs, without requiring the ESP-IDF-gated register() itself.
// ---------------------------------------------------------------------------
#ifdef BB_DIAG_STORAGE_NVS_TESTING
#include "bb_diag_storage_nvs_test.h"

bb_err_t bb_diag_storage_nvs_assemble_schema_for_test(void)
{
#if defined(CONFIG_BB_OPENAPI_RUNTIME_META)
...
const char *bb_diag_storage_nvs_get_describe_schema_for_test(void)
{
    return s_diag_storage_nvs_describe_responses[0].schema;
}
#endif /* BB_DIAG_STORAGE_NVS_TESTING */
