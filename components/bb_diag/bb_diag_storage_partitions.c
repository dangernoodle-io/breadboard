...
static const char *TAG = "bb_diag_storage_partitions";
...
// call is a pointer-stable no-op, guarded by
...
static bb_err_t ensure_schema_patched(void)
{
    bb_err_t rc = bb_serialize_meta_ensure_composed(bb_serialize_meta_openapi_schema,
                                                      &bb_diag_storage_partitions_desc, &bb_diag_storage_partitions_meta,
                                                      s_diag_storage_partitions_schema_buf,
                                                      sizeof(s_diag_storage_partitions_schema_buf));
...
bb_err_t bb_diag_storage_partitions_register(void)
{
    ...
    // storage/partitions section (and its dispatch-table entry) offline --
    // degrade by leaving the describe-route schema unpatched (NULL, omitted
    // by bb_openapi_emit.c's pointer-NULL gate) and register anyway.
    ...
    if (schema_rc != BB_OK) {
        bb_log_w(TAG, "storage/partitions schema compose failed (rc=%d), describe route ships without a schema", (int)schema_rc);
    }
#endif
