...
// ---------------------------------------------------------------------------
// GET /api/diag/panic response schema (B1-1059 emit batch C, site C1).
// Config OFF (default) keeps BB_DIAG_PANIC_GET_SCHEMA_LITERAL byte-identical
// -- config ON composes it at init from bb_diag_panic_get_wire_desc/
// bb_diag_panic_get_wire_meta via bb_serialize_meta_ensure_composed() -- see
// bb_diag_panic_get_wire_ensure_schema_patched() below. Accepted config-ON
// delta (see test_bb_diag_panic_get_wire_meta_golden.c): the composed body
...
// (test_bb_diag_panic_get_wire_meta_golden.c's k_expected_meta_schema is 416
...
// silently truncating.
static char s_diag_panic_get_schema_buf[512];
#else
static const char k_diag_panic_get_schema[] = BB_DIAG_PANIC_GET_SCHEMA_LITERAL;
...
bb_err_t bb_diag_panic_get_wire_ensure_schema_patched(void)
{
    return bb_serialize_meta_ensure_composed(bb_serialize_meta_openapi_schema,
                                              &bb_diag_panic_get_wire_desc, &bb_diag_panic_get_wire_meta,
                                              s_diag_panic_get_schema_buf, sizeof(s_diag_panic_get_schema_buf));
}
#endif /* CONFIG_BB_OPENAPI_RUNTIME_META */
...
{
#if defined(CONFIG_BB_OPENAPI_RUNTIME_META)
    return s_diag_panic_get_schema_buf;
#else
    return k_diag_panic_get_schema;
#endif /* CONFIG_BB_OPENAPI_RUNTIME_META */
}
