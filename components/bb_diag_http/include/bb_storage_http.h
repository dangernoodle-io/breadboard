...
// descriptor (bb_storage_http_routes.c's file-scope s_storage_delete_desc),
// same #if-gated pattern as bb_storage_http_factory_reset_meta below
// (B1-1059 PR-2b-i-1). BB_SERIALIZE_META_HOST is a host-only define (set by
// the PlatformIO native env; see platformio.ini) -- NEVER set by the
// ESP-IDF/device build, so these two declarations (and their definitions
// in bb_storage_http_routes.c) compile to nothing on-device. The desc
