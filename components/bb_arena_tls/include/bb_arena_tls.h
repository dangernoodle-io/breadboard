...
/**
 * bb_arena_tls — boot-reserved contiguous mbedTLS handshake arena.
 *
 * Consumer of the generic bb_arena primitive (components/bb_arena): the
 * arena mechanics (bump-alloc, owns, reset, stats) live in bb_arena; this
 ...
 *   Call bb_arena_tls_init() at the very top of app_main(), BEFORE
 ...
 *   - Installs bb_arena_tls_calloc/bb_arena_tls_free as the global mbedTLS
 *     allocator via mbedtls_platform_set_calloc_free().
 *   - If CONFIG_BB_ARENA_TLS_BYTES > 0: reserves a static contiguous buffer
 *     of that size (via bb_arena_init on a static BSS buffer) and serves
 *     allocations arena-first (try-arena, fall back to
 *     bb_calloc_internal()/bb_mem_free() when exhausted).
 *   - If CONFIG_BB_ARENA_TLS_BYTES == 0: no arena buffer; calloc/free route
 ...
 * arena/counter, not given independent arenas — size CONFIG_BB_ARENA_TLS_BYTES
 * for cumulative concurrent usage (see Kconfig help).
 *
 * bb_arena_tls_init() is idempotent: a second call (e.g. an explicit
 * app_main() call plus CONFIG_BB_ARENA_TLS_AUTOREGISTER) is a true no-op and
 ...
 *
 * REUSE ACROSS HANDSHAKES: the underlying bb_arena is a bump allocator that
 * does not reclaim on individual frees. This component tracks outstanding
 * arena allocations and calls bb_arena_reset() only once that count drains
 ...
 * region. Size CONFIG_BB_ARENA_TLS_BYTES for the cumulative allocation of a
