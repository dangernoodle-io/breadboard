...
| [bb_wifi_prov](./bb_wifi_prov/) | bb_wifi_prov — Wi-Fi provisioning HTTP routes: parses a POSTed SSID/password form and a captive-portal redirect. Registers POST /save and a shared GET /* wildcard on the shared HTTP server that serves consumer assets plus the captive-portal redirect as its no-match fallback; does not register /api/version, /api/wifi/scan, or /api/reboot (those live in bb_wifi_http / bb_system), and does not itself bring up SoftAP or drive a Wi-Fi lifecycle state machine (see bb_wifi_ap for AP bring-up). |
